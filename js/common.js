...
var task_url = 'http://localhost:32500/Task/'
...
function signIn(e)
{
	...
	axios.post(customer_login_url, data, axiosConfig)
    .then(response => {
...
function signUp(e)
{
	...
	axios.post(customer_url, data,axiosConfig)
    .then(response => {
    ...
    */
    ...
    function checkout(e){
      ...
      console.log(sessionStorage.products);
		...
		products.forEach(function (product){
			let data = {};
			data.id = getRandomInt(product.qty);
			data.description = product.details;
			data.priority = product.id;
