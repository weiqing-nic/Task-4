...
function submitInfo(){
    ...
    console.log(myTasks[myTasks.length - 1]);
    var obj = myTasks[myTasks.length - 1];
    var NiceRandomNumber = Math.floor(Date.now() + Math.random())  % 1147483647;
    console.log(NiceRandomNumber);
    ...
    fetch("http://localhost:32500/Task", requestOptions)
    .then(response => response.text())
    .then(result => console.log(result))
    .catch(error => console.log('error', error));

	
	// fetch('http://localhost:32500/Task',
    // {
    //     method: 'POST', // or 'PUT'
    //     headers: {
